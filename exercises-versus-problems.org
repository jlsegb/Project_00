...
****  paragraph explaning difference between excercise and a problem.

As I understand, there's a difference between excercises and problems and that is the mental effort involved. Both require mental effort, but in a different way.
One is methodical while the other is yet to be defined. I believe that what we are learning now is a problem, until we've learned it and understood it. By the point that we 
have learned about it and understand the concepts, then it becomes a excercise. If excercising a specific subject becomes frequent and regular, then we can master 
that specific subject. But after there is understanding it becomes an excercise. We can apply the method to solve the problem over and over again.
In a way learning to be a problem solver is higher than an excercise solver. A good reason for that would be that we need problem solvers (AKA programmers) to solve
a problem (write software) that will sove excercises (A methodical proccess). That way we have calculators that solve excercises, but there was a need for a 
problem solver to create the circuitry, write the program, interface the analog and digital pieces and bug test it. Being a problem solver is a desired
skill that most companies look for, they have software to solve their excercises.
